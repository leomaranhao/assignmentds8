...
Three methods will be applied to model the classifications (with the trainingPart dataset). These 3 methods will be combined using a random forest to build the final model that will be used for the quiz predictions. The methods are: Generalized Boosted Model, Random Forests and Linear Discriminant Analysis.
